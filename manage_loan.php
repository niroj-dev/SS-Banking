	...
	<form action="" id="loan-application">
		...
		<div class="row">
				<div class="col-md-6">
					...
					<?php
					// Corrected SQL query
					$borrower = $conn->query("SELECT *, CONCAT(b.C_Last_Name, ', ', b.C_First_Name) AS name FROM customer_detail b");
					?>
					<select name="borrower_id" id="borrower_id" class="custom-select browser-default select2">
						<option value=""></option>
						<?php while($row = $borrower->fetch_assoc()): ?>
							<option value="<?php echo $row['C_No'] ?>" <?php echo isset($borrower_id) && $borrower_id == $row['C_No'] ? "selected" : '' ?>>
								<?php echo $row['name'] . ' | Account_No:' . (isset($row['Account_No']) ? $row['Account_No'] : 'N/A') ?>
							</option>
						<?php endwhile; ?>
					</select>
				</div>
...
<style>
	...
	#uni_modal .modal-dialog {
        max-width: 90%; /* Increase width as needed */
