				...
				<table class="table table-bordered" id="loan-list">
					...
					<tbody>
						<?php
							...
							$qry = $conn->query("SELECT l.*,concat(b.C_Last_Name,', ',b.C_First_Name,' ',b.C_Last_Name)as name, b.C_Mobile_No, b.C_Pincode from loan_list l inner join customer_detail b on b.C_No = l.borrower_id  order by id asc");
							while($row = $qry->fetch_assoc()):
								...
								$offset = $paid > 0 ? " offset $paid ": "";
								    if ($row['status'] == 2):
        $next_schedule_result = $conn->query("SELECT * FROM loan_schedules 
                                             WHERE loan_id = '" . intval($row['id']) . "' 
                                             ORDER BY DATE(date_due) ASC LIMIT 1 $offset");
        $next_schedule = $next_schedule_result->fetch_assoc();
        $next = $next_schedule ? $next_schedule['date_due'] : null;
    endif;
								$sum_paid = 0;
